#include "AES.h"
#include<bits/stdc++.h>
using namespace std;

const uint8_t sbox[256] = {
    };

const uint8_t inv_sbox[256] = {
    };

const uint8_t rcon[11] = {0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36};

void SubBytes(uint8_t *state)
{
    for (int i = 0; i < 16; ++i)
        state[i] = sbox[state[i]];
}

void InvSubBytes(uint8_t *state)
{
    for (int i = 0; i < 16; ++i)
        state[i] = inv_sbox[state[i]];
}
void ShiftRows(uint8_t *state)
{
    uint8_t temp[16];

    // Row 0 (no shift)
    temp[0] = state[0];
    temp[4] = state[4];
    temp[8] = state[8];
    temp[12] = state[12];

    // Row 1 (shift left by 1)
    temp[1] = state[5];
    temp[5] = state[9];
    temp[9] = state[13];
    temp[13] = state[1];

    // Row 2 (shift left by 2)
    temp[2] = state[10];
    temp[6] = state[14];
    temp[10] = state[2];
    temp[14] = state[6];

    // Row 3 (shift left by 3)
    temp[3] = state[15];
    temp[7] = state[3];
    temp[11] = state[7];
    temp[15] = state[11];

    // Copy temp back to state
    for (int i = 0; i < 16; ++i)
    {
        state[i] = temp[i];
    }
}

void InvShiftRows(uint8_t *state)
{
    uint8_t temp[16];

    // Row 0 (no shift)
    temp[0] = state[0];
    temp[4] = state[4];
    temp[8] = state[8];
    temp[12] = state[12];

    // Row 1 (shift right by 1)
    temp[1] = state[13];
    temp[5] = state[1];
    temp[9] = state[5];
    temp[13] = state[9];

    // Row 2 (shift right by 2)
    temp[2] = state[10];
    temp[6] = state[14];
    temp[10] = state[2];
    temp[14] = state[6];

    // Row 3 (shift right by 3)
    temp[3] = state[7];
    temp[7] = state[11];
    temp[11] = state[15];
    temp[15] = state[3];

    for (int i = 0; i < 16; ++i)
        state[i] = temp[i];
}

uint8_t xtime(uint8_t x)
{
    return (x << 1) ^ (((x >> 7) & 1) * 0x1b);
}

void MixColumns(uint8_t *state)
{
    for (int i = 0; i < 4; ++i)
    {
        uint8_t *col = &state[i * 4];
        uint8_t a = col[0], b = col[1], c = col[2], d = col[3];

        uint8_t ab = a ^ b, bc = b ^ c, cd = c ^ d, da = d ^ a, ac = a ^ c, bd = b ^ d;

        uint8_t temp = a ^ b ^ c ^ d;

        col[0] = a ^ xtime(a ^ b) ^ temp;
        col[1] = b ^ xtime(b ^ c) ^ temp;
        col[2] = c ^ xtime(c ^ d) ^ temp;
        col[3] = d ^ xtime(d ^ a) ^ temp;
    }
}

uint8_t mul(uint8_t a, uint8_t b)
{
    uint8_t result = 0;
    while (b)
    {
        if (b & 1)
            result ^= a;
        a = xtime(a);
        b >>= 1;
    }
    return result;
}

void InvMixColumns(uint8_t *state)
{
    for (int i = 0; i < 4; ++i)
    {
        uint8_t *col = &state[i * 4];
        uint8_t a = col[0], b = col[1], c = col[2], d = col[3];

        col[0] = mul(a, 0x0e) ^ mul(b, 0x0b) ^ mul(c, 0x0d) ^ mul(d, 0x09);
        col[1] = mul(a, 0x09) ^ mul(b, 0x0e) ^ mul(c, 0x0b) ^ mul(d, 0x0d);
        col[2] = mul(a, 0x0d) ^ mul(b, 0x09) ^ mul(c, 0x0e) ^ mul(d, 0x0b);
        col[3] = mul(a, 0x0b) ^ mul(b, 0x0d) ^ mul(c, 0x09) ^ mul(d, 0x0e);
    }
}

void AddRoundKey(uint8_t *state, const uint8_t *roundKey)
{
    for (int i = 0; i < 16; ++i)
        state[i] ^= roundKey[i];
}

// Rotate word: [a0, a1, a2, a3] → [a1, a2, a3, a0]
void RotWord(uint8_t *word)
{
    uint8_t temp = word[0];
    word[0] = word[1];
    word[1] = word[2];
    word[2] = word[3];
    word[3] = temp;
}

// SubWord using S-box
void SubWord(uint8_t *word)
{
    for (int i = 0; i < 4; ++i)
    {
        word[i] = sbox[word[i]];
    }
}

void KeyExpansion(const uint8_t *key, uint8_t *roundKeys)
{
    // Generate 11 round keys from the original key
    memcpy(roundKeys, key, 16); // The memcpy() function is used to copy a block of memory from one location to another.

    int bytesGenerated = 16; // We've filled in the first 16 bytes
    int rconIteration = 1;
    uint8_t temp[4];

    while (bytesGenerated < 176)
    {
        // Copy previous 4 bytes into temp
        for (int i = 0; i < 4; ++i)
            temp[i] = roundKeys[bytesGenerated - 4 + i];

        if (bytesGenerated % 16 == 0)
        {
            RotWord(temp);
            SubWord(temp);
            temp[0] ^= rcon[rconIteration++];
        }

        for (int i = 0; i < 4; ++i)
        {
            roundKeys[bytesGenerated] = roundKeys[bytesGenerated - 16] ^ temp[i];
            ++bytesGenerated;
        }
    }
}

void PadMessage(vector<uint8_t> &message)
{
    int padLen = 16 - (message.size() % 16);
    message.insert(message.end(), padLen, padLen);
}

void AESBlockEncrypt(uint8_t *block, uint8_t *roundKeys)
{
    AddRoundKey(block, roundKeys);
    for (int round = 1; round <= 9; ++round)
    {
        SubBytes(block);
        ShiftRows(block);
        MixColumns(block);
        AddRoundKey(block, roundKeys + round * 16);
    }
    SubBytes(block);
    ShiftRows(block);
    AddRoundKey(block, roundKeys + 10 * 16);
}

vector<uint8_t> AESEncrypt(const vector<uint8_t> &data, const vector<uint8_t> &key)
{

    if (key.size() != 16)
        throw runtime_error("Invalid AES key size");

    vector<uint8_t> result = data;
    PadMessage(result);

    uint8_t roundKeys[176];
    KeyExpansion(key.data(), roundKeys);

    for (size_t i = 0; i < result.size(); i += 16)
    {
        AESBlockEncrypt(result.data() + i, roundKeys);
    }
    return result;
}

void AESBlockDecrypt(uint8_t *block, uint8_t *roundKeys)
{
    AddRoundKey(block, roundKeys + 10 * 16);
    for (int round = 9; round >= 1; --round)
    {
        InvShiftRows(block);
        InvSubBytes(block);
        AddRoundKey(block, roundKeys + round * 16);
        InvMixColumns(block);
    }
    InvShiftRows(block);
    InvSubBytes(block);
    AddRoundKey(block, roundKeys);
}

void RemovePadding(vector<uint8_t> &data)
{
    uint8_t pad = data.back();
    if (pad > 0 && pad <= 16)
    {
        bool valid = true;
        for (int i = 1; i <= pad; ++i)
            if (data[data.size() - i] != pad)
                valid = false;
        if (valid)
            data.resize(data.size() - pad);
    }
}

vector<uint8_t> AESDecrypt(const vector<uint8_t> &data, const vector<uint8_t> &key)
{
    if (key.size() != 16)
        throw runtime_error("Invalid AES key size");

    vector<uint8_t> result = data;
    uint8_t roundKeys[176];
    KeyExpansion(key.data(), roundKeys);

    for (size_t i = 0; i < result.size(); i += 16)
    {
        AESBlockDecrypt(result.data() + i, roundKeys);
    }
    RemovePadding(result);
    return result;
}
vector<uint8_t> GenerateAESKey()
{
    return {0x2b, 0x7e, 0x15, 0x16,
            0x28, 0xae, 0xd2, 0xa6,
            0xab, 0xf7, 0x15, 0x88,
            0x09, 0xcf, 0x4f, 0x3c}; // Example AES-128 key
}
